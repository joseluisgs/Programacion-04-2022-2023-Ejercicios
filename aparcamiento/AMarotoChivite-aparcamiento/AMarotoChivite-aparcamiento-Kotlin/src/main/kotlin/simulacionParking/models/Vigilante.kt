package simulacionParking.models

import simulacionParking.enums.Color
import simulacionParking.enums.TipoVehiculo

class Vigilante {

    companion object {
        // No queremos cambiar el nombre del Vigilante, por ello final
        const val name = "Angel"

        /**
         * Imprime en pantalla el Vigilante presentándose, donde puede aparcar el cliente/conductor
         *
         * @param tipo     para especificar al cliente/conductor que tipo de vehículo puede aparcar
         * @param filas    para especificar al cliente/conductor en que fila puede aparcar
         * @param columnas para especificar al cliente/conductor en que columna puede aparcar
         */
        fun indicarDondeAparcar(tipo: TipoVehiculo, filas: Int, columnas: Int) {
            println(
                (Color.PURPLE_UNDERLINED.get() + "VIGILANTE" + Color.RESET.get()).toString() + "-> Hola soy " + name + " puedes aparcar tu " + tipo.get() + " en el parking fila:" + (filas + 1) + " columna:" + (columnas + 1) +
                        " por un precio de 3.75€"
            )
        }

        /**
         * Verifica que si está lleno el parking, si está lleno, el Vigilante avisará
         *
         * @param parking, para poder verificar en qué parking
         * @return false = si no está completo, true = está completo
         */
        fun parkingCompleto(parking: Parking): Boolean {
            for (filas in parking.matriz.indices) {
                for (columnas in parking.matriz[filas].indices) {
                    if (parking.matriz[filas][columnas]!!.tipo == TipoVehiculo.VACIO) {
                        return false
                    }
                }
            }
            println(parking)
            println((Color.PURPLE_UNDERLINED.get() + "VIGILANTE" + Color.RESET.get()).toString() + "-> Hola soy " + name + ", no puedes aparcar, tenemos el Parking completo!")
            return true
        }

        /**
         * El vigilante ha terminado su jornada y ha acabado el programa sin ninguna incidencia
         */
        fun trabajoTerminado() {
            println("$name ha hecho un buen trabajo de vigilante y ha terminado su jornada")
        }
    }
}