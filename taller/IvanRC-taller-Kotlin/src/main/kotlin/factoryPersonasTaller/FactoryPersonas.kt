...
class FactoryPersonas {
    ...
    fun createRandomPersona(): Persona?{
        ...
        if(chance <= 10){
            persona = Persona.JefeTaller(nombres.random(), añosExepriencia, personasACargo)
        }else{
            if(chance <= 60){
                persona = Persona.Trabajador(nombres.random(), añosExepriencia, horasDiarias)
            }else{
                if(chance <= 80){
                    persona = Persona.Chapista(nombres.random(), añosExepriencia, horasDiarias)
                }else{
                    if(chance <= 90) {
                        persona = Persona.Electricista(nombres.random(), añosExepriencia, horasDiarias)
                    }else{
                        persona = Persona.NavajaSuiza(nombres.random(), añosExepriencia, horasDiarias)
                    }
                }
            }
        }
